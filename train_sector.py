...
from sector.config import get_args
from sector.dataset import split_data
...
def main():
    ...
    while True:
        ...
        if accuracy > max_test_acc and train_acc > 0.8:
    ...
    output_file = args.output + '_tgt' + str(args.tgt_class) + '_' + now + '.pkl'
    model_file = args.model + '_tgt' + str(args.tgt_class) + '.pth'
    with open(output_file, 'wb') as f:
        pickle.dump(best_result, f)
    torch.save(best_model, args.model)
    with open('tmp/sector_memory_tgt'+str(args.tgt_class)+'_'+now+'.pkl', 'wb') as f:
